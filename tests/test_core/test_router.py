...
def test_build_router_with_all_integrations(mock_agent: MockAgent):
...
def test_build_router_webhook_only(mock_agent: MockAgent):
...
def test_build_router_no_integrations(mock_agent: MockAgent):
    """Test building router with no integrations."""
    router = build_router(mock_agent, slack=None, discord=None, webhook=False)
...
def test_webhook_endpoint(test_client: TestClient):
    ...
    response = test_client.post("/agent/webhook", json={"message": "test"})

    assert response.status_code == 200
    assert response.json() == {"content": "Mock webhook response", "metadata": None}


@pytest.mark.asyncio
async def test_webhook_handler_error(mock_agent: MockAgent, test_client: TestClient):
